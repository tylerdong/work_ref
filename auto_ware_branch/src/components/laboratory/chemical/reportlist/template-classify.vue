...
<template>
  <div>
    ...
    <add-dialog ref="addDialog" :formData="formData" @loadData="getData"></add-dialog>
  </div>
</template>
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    methods: {
      ...
      edit (listItem) {
        this.formData.title = '修改'
        this.formData.name = listItem.row.name
        this.formData.id = listItem.row.id
        this.$refs.addDialog.show()
      },
      getData () {
        let params = {
