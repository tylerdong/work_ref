      ...
      <div class="flex-div-row" style="background: white">
        <div class="flex-div-column hy-admin__search-main cf" ref="container">
          ...
          <div class="fr" style="margin-bottom: 20px">
            ...
            <el-button @click="add" type="primary">校准登记</el-button>
          </div>
          <div>
          ...
          <instrument-adjusting-dialog ref="dialog" :groupOptions="options.group" @success="success"></instrument-adjusting-dialog>
        </div>
      </div>
    </div>
  </div>
...
<script>
  ...
  export default {
    components: {
      'instrument-adjusting-dialog': require('./instrument-adjusting-dialog.vue')
    },
    data () {
      return {
        searchInfo: {
    ...
    methods: {
      ...
      remove (scope) {
        this.$confirm('是否删除?', {type: 'warning'}).then(() => {
          api.chemicalLaboratory.labInstrumentCalibration.deleteLabInstrumentCalibrationDo({
            id: scope.row.id,
      ...
      getListData () { // 获取列表结构
        ...
        api.chemicalLaboratory.labInstrumentCalibration.getLabInstrumentCalibrationDoList(params).then(response => {
          const data = response.data
          if (data.success === true) {
            if (!data.data) {
              this.tableData = []
