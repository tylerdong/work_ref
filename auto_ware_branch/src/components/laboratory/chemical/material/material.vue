    ...
    methods: {
      ...
      remove (scope) {
        this.$confirm('是否删除?', {type: 'warning'}).then(() => {
          api.chemicalLaboratory.labMaterialController.deleteLabMaterialDo({
            id: scope.row.id,
      ...
      getListData () { // 获取列表结构
        ...
        api.chemicalLaboratory.labMaterialController.getLabMaterialDoList(params).then(response => {
          const data = response.data
          if (data.success === true) {
            if (!data.data) {
              this.tableData = []
