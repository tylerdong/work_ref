<!--新增，修改样品-->
<template>
  <el-dialog :title="form.title" :visible.sync="dialogToggle" @close="close" class="template-dialog" width="50%">
  ...
  export default {
    ...
    methods: {
        ...
        if (this.form.title === '新增') {
          ...
          this.$refs[ruleForm].validate((valid) => {
            if (valid) {
              this.dialogToggle = true
              console.log(params)
              api.chemicalLaboratory.labSampleManagement.createLabSampleManagementDo(params).then(response => {
          ...
          this.$refs[ruleForm].validate((valid) => {
            if (valid) {
              this.dialogToggle = true
              api.chemicalLaboratory.labSampleManagement.updateLabSampleManagementDo(JSON.stringify(params)).then(response => {
                const data = response.data
                if (data.success === true) {
                  this.dialogVisible = false
      ...
      getTemplateDoList () {
        ...
        api.chemicalLaboratory.labRptTemplateController.getLabRptTemplateDoList(param).then(response => {
          const data = response.data
          if (data.success === true) {
