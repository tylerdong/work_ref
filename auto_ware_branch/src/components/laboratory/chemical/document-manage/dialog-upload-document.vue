<!--编辑实验报告单模板信息-->
<template>
  <el-dialog title="上传文件" :visible.sync="dialogToggle" @close="close" class="edit-dialog-template-dialog" width="30%">
    ...
    methods: {
      ...
      handleSelectFileDeal () {
        ...
        if (['pdf', 'docx', 'doc', 'xlsx', 'xls'].includes(type)) {
          ...
          api.chemicalLaboratory.labFileController.saveLabFile(formData).then((response) => {
