  ...
  <div>
    <div class="hy-admin__main-container">
      <div class="main-left">
        <aside>
          <div>
            <el-select v-model="defaultSelection" placeholder="请选择" @change="getTreeData" class="select-box">
        ...
        <div class="flex-div-column hy-admin__search-main cf">
          ...
          <el-table :data="tableData" border v-loading="loading.list" element-loading-text="拼命加载中" style="margin-top: 20px">
            ...
            <el-table-column label="操作" width="165">
              <template slot-scope="scope">
                <el-button @click="cancel(scope)" type="primary" size="text" :loading="loading.cancel">取消</el-button>
                <el-button @click="doExperiment(scope)" type="primary" size="text" :loading="loading.doExperiment">发布</el-button>
              </template>
            </el-table-column>
          </el-table>
    ...
    <dialog-publish-experiment ref="refPublishExperiment" @initExperiment="initExperiment"></dialog-publish-experiment>
  </div>
</template>
<script>
  ...
  export default {
    components: {
      'dialog-publish-experiment': require('./dialog-publish-experiment.vue')
    },
    created () {},
    data () {
      return {
    ...
    methods: {
      ...
      getTreeData () {
        ...
        api.chemicalLaboratory.labRptRecordController.getLabSampleManagementGroupVoByProcessingRptRecords(params).then(response => {
      ...
      getListData () { // 获取列表结构
        ...
        api.chemicalLaboratory.labRptRecordController.getLabRptRecordDoList4Processing(params).then(response => {
          const data = response.data
          if (data.success === true) {
            if (!data.data) {
              this.tableData = []
        ...
        this.$confirm(`是否取消？`, '提示', {
          ...
          beforeClose: (action, instance, done) => {
            if (action === 'confirm') {
              ...
              api.chemicalLaboratory.labRptRecordController.cancelRptRecord(params).then(response => {
                const data = response.data
                if (data.success === true) {
                  done()
