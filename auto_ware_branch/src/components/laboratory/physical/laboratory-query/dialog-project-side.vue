        ...
        <el-form :model="form" ref="templateForm" label-width="100px" v-loading="loading.form"
          ...
          <el-form-item v-for="item in form.dataArray" :key="item.nodeCode"
            ...
            <el-button v-else-if="item.type === 'TABLE'" :name="item.nodeCode" size="small" type="primary"
                       @click="getTableValues(item.nodeCode)">计算table评定
            </el-button> -->
            <!--输入框下面的文字-->
      ...
      dataProcess (jsonArray) {
        ...
        for (let i = 0; i < jsonArray.length; i++) {
          ...
          if (jsonArray[i].hasOwnProperty('documentType') && jsonArray[i].documentType === 'TEMP_SELECT' && jsonArray[i].selectStaticMapId) {
            let maps = this.tempSelectStaticMaps.find(item => item.key === jsonArray[i].selectStaticMapId)
            if (Object.is(maps, undefined)) {
              api.physicalLaboratory.labSelectStaticMap.getLabSelectStaticMapDosByParentId({parentId: jsonArray[i].selectStaticMapId}).then((response) => {
                ...
                if (data.success) {
                  ...
                  this.tempSelectStaticMaps.push({key: jsonArray[i].selectStaticMapId, value: data.data})
                }
              })
//              $.ajax({
//                type: 'GET',
//                dataType: 'json',
//                async: false,
//                url: window.global.ajaxBaseUrl + 'api/lab/report/labSelectStaticMapController/getLabSelectStaticMapDosByParentId?parentId=' + jsonArray[i].selectStaticMapId,
//                success: (response) => {
//                  let data = response.data
//                  if (data.success) {
//                    jsonArray[i].selectStaticMaps = data.data
...
//              })
              ...
              jsonArray[i].selectStaticMaps = maps
            }
          }
      ...
      getTableRef (nodeCode, refTableCode) {
        let dataItem = this.form.dataArray.find(arr => { return arr.nodeCode === refTableCode })
        let param = {
          source: 'LAB_ORIGINAL',
          tableAttributeIndexJsonVo: dataItem.tableLines[0].find(line => { return line.type === 'TABLE_ATTRIBUTE_INDEX' }),
          tableLines: dataItem.tableLines
        }
        api.physicalLaboratory.labIndexEvaluationController.groupIndexValue(param).then(response => {
          const data = response.data
          if (data.success === true) {
            let currentDataItem = this.form.dataArray.find(arr => { return arr.nodeCode === nodeCode })
      ...
      getTargetVal (nodeCode) {
        let params = {
          source: 'LAB_ORIGINAL',
          labBaseJsonVo: this.form.dataArray.find(arr => { return arr.nodeCode === nodeCode }),
      ...
      getTableValues (nodeCode) {
        let dataItem = this.form.dataArray.find(arr => { return arr.nodeCode === nodeCode })
        let tableAttribute = dataItem.tableLines[0].find(line => { return line.type === 'TABLE_ATTRIBUTE_INDEX' })
        if (tableAttribute === undefined) {
          this.$message.error(`没有设置指标评定，无须评定`)
          return
        }
        let param = {
          source: 'LAB_ORIGINAL',
        ...
        api.physicalLaboratory.labIndexEvaluationController.calAllIndexValue(param).then(response => {
          const data = response.data
          if (data.success === true) {
            dataItem = this.form.dataArray.find(arr => { return arr.nodeCode === nodeCode })
            dataItem.tableLines.forEach((lines, linesIndex) => {
              let attribute = lines.find(line => line.type === 'TABLE_ATTRIBUTE_INDEX')
              attribute.value = data.data[linesIndex]
            })
          } else {
            this.$message.error(data.errorMsg)
          }
