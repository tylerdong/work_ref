...
<script type="text/ecmascript-6">
  ...
  export default {
    components: {
      'add-dialog': require('./delete-or-change.vue')
    },
    created () {},
    data () {
      return {
    ...
    methods: {
      ...
      del (item) {
        this.$confirm('是否删除该分类, 是否继续?', '提示', {
          ...
          let params = {id: item.row.id, modifier: item.row.modifier}
          api.physicalLaboratory.classify.deleteLabDataGroupDicDo(JSON.stringify(params)).then(response => {
            let data = response.data
            if (data.success) {
              this.$message.success('删除成功')
              this.getData()
              eventHub.$emit('documentTypeChange')
            } else {
