...
<template>
  <div>
      ...
      <div class="flex-div-row" style="background: white">
        <div class="flex-div-column hy-admin__search-main cf">
          <el-tabs @tab-click="tabClick" :value="option.tabs[0].name">
            <el-tab-pane v-for="(item,index) in option.tabs" :key="index" :label="item.label" :name="item.name">
              <div class="fr" style="margin-bottom: 20px">
                ...
                <!--<el-autocomplete v-show="!isPlan" v-model="search.trainingPlan" :fetch-suggestions="suggestTrainPlan"-->
                                 <!--@select="selectTrainPlan" placeholder="请输入名称"></el-autocomplete>-->
    ...
    <el-dialog title="培训登记" width="80%" :visible.sync="visible.registerDialog">
  ...
  export default {
    ...
    data () {
      return {
        ...
        templateId: '',
        option: {tabs: [{label: '培训计划', name: 'plan'}, {label: '培训记录', name: 'record'}], users: []},
    ...
    methods: {
      ...
      deletePlan (scope) {
        this.$confirm('是否删除该培训计划, 是否继续?', '提示', {
          ...
          api.physicalLaboratory.labTrainingPlanController.deleteLabTrainingPlanVo(params).then(response => {
      ...
      confirmRegister (formName) {
        ...
        this.$refs[formName].validate(valid => {
          if (valid) {
            ...
            if (this.form.id > 0) {
              api.physicalLaboratory.labTrainingRecordController.updateLabTrainingRecordVo(this.form).then(response => {
                const data = response.data
                if (data.success === true) {
                  this.gePlantListData()
                } else {
              ...
              api.physicalLaboratory.labTrainingRecordController.createLabTrainingRecordVo(this.form).then(response => {
                const data = response.data
                if (data.success === true) {
                  this.gePlantListData()
                } else {
      ...
      deleteRecord (scope) {
        this.$confirm('是否要删除此记录', '提示', {
          ...
          api.physicalLaboratory.labTrainingRecordController.deleteLabTrainingRecordVo({
            id: scope.row.id,
      ...
      gePlantListData () { // 获取列表结构
        ...
        api.physicalLaboratory.labTrainingPlanController.getLabTrainingPlanDoList(params).then(response => {
          const data = response.data
          if (data.success === true) {
            if (!data.data) {
              this.tableData.plan = []
      ...
      suggestTrainPlan () {
        ...
        api.physicalLaboratory.labTrainingPlanController.getLabTrainingPlanDosByTrainingTile(params).then(response => {
      ...
      getRecordListData () {
        ...
        api.physicalLaboratory.labTrainingRecordController.getLabTrainingRecordDoList(param).then(response => {
          let data = response.data
          if (!data.data) {
