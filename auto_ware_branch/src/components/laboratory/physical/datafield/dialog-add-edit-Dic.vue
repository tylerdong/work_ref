<!--新增字典-->
<template>
  <el-dialog title="字典" :visible.sync="dialogToggle" @close="close" class="edit-dialog-template-dialog"
             width="50%">
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            if (this.form.id) {
              ...
              api.physicalLaboratory.labSelectStaticMap.updateLabSelectStaticMapDo(params).then((response) => {
              ...
              api.physicalLaboratory.labSelectStaticMap.createLabSelectStaticMapDo(params).then((response) => {
                let data = response.data
                if (data.success) {
                  this.close()
                  this.$emit('initData')
