  ...
  <div ref="dialogMain">
    <jk-dialog :title="form.title" :visible.sync="dialogVisible" width="50%">
      ...
      <div class="hy-admin__search-main cf">
        <div class="fr">
          <el-button @click="judgment" type="primary" :loading="loading.judgment">一键评定</el-button>
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    methods: {
      ...
      judgment () {
        this.loading.judgment = true
        let tableAttribute = this.form.dataArray.filter(line => { return line.type === 'REF_INDEX_EVALUATION' })
        if (tableAttribute === undefined) {
          this.$message.error(`没有设置指标评定，无须评定`)
          return
        }
        tableAttribute.forEach(item => { item.value = '' })
        let param = {
          source: 'LAB_RPT',
          ...
          tableAttributeIndexJsonVos: tableAttribute
        }
        api.physicalLaboratory.labIndexEvaluationController.calAllIndexValueOneKey(param).then(response => {
          const data = response.data
          if (data.success === true) {
            for (let i = 0; i < data.data.length; i++) {
              let evaluation = this.form.dataArray.find(item => item.nodeCode === data.data[i].nodeCode)
              evaluation.value = data.data[i].value
              evaluation.standardValue = data.data[i].standardValue
            }
          } else {
            this.$message.error(data.errorMsg)
          }
