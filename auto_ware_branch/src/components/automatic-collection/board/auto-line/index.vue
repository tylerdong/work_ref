<template>
  <div class="content">
    <div class="all-wrapper1">
      <el-form :inline="true" :rules="rules" ref="form" :model="search">
      ...
      showClick () {
        this.$refs.form.validate(valid => {
          if (valid) {
            ...
            let params = { groupId: boardConfig.autoLine.groupId }
            api.automatic.statement.getBoardConfig(params).then(response => {
              const data = response.data
              if (data.messageType === 1) {
                if (Array.isArray(data.data) && data.data.length > 0) {
                  console.log(this.boardConfig)
                  this.boardConfig.refreshInterval = data.data[0].list[0].requestInterval
                  clearInterval(this.timeinter)
                  this.timeinter = setInterval(this.getData, this.boardConfig.refreshInterval * 1000)
                }
              } else {
