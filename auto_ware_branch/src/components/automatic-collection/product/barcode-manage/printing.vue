...
<template>
  <div class="all-wrapper">
    <div class="action-bar">
      <el-form :inline="true">
        <el-form-item label="状态">
          <el-checkbox v-model="search.printflag">未打印</el-checkbox>
        </el-form-item>
        <el-form-item label="线别">
          <el-select v-loading="loading.line" v-model="search.line" placeholder="请选择线别" clearable class="input-item">
        ...
        <el-form-item label="批号">
          <el-select v-loading="loading.batchNum" v-model="search.batchNum" filterable placeholder="请选择批号" clearable
                     class="input-item">
            <el-option v-for="item in batchNumOptions" :key="item.batchNo" :label="item.batchNo" :value="item.batchNo">
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="班次">
          <el-select v-loading="loading.classes" v-model="search.classes" placeholder="请选择班次" clearable class="input-item">
            <el-option v-for="item in classOptions" :key="item.id" :label="item.name" :value="item.id">
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="日期">
          <el-date-picker v-model="search.dateValue" type="date" placeholder="请选择日期" class="input-item"></el-date-picker>
        </el-form-item>
        <el-form-item>
    ...
    <div class="main-wrappper">
      ...
      <!--</div>-->
      <el-table v-loading="loading.list" :data="tableData" border @selection-change="handleSelectionChange" style="width: 100%">
        ...
        <el-table-column label="落筒方式">
          <template slot-scope="scope">
            <span>{{ scope.row.fallType === '1'?'手动落筒':'自动落筒' }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="fallNo" label="落次"></el-table-column>
        <el-table-column prop="classesName" label="班次"></el-table-column>
        <el-table-column prop="silkNum" label="锭数"></el-table-column>
        <el-table-column prop="num" label="总锭数"></el-table-column>
    ...
    <dialog-printing-update ref="dialogPrintingUpdate" :classOptions="classOptions" @submitSuccess="getData"></dialog-printing-update>
    ...
    methods: {
      batchUpdateClick () {
        if (!this.multipleSelection.length) {
          return this.$message({message: '请选择数据'})
        }
