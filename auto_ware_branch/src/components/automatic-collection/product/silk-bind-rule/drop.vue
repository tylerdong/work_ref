    ...
    <div class="hy-admin__main-container">
      ...
      <el-form :inline="true" label-width="8rem" class="form-padding">
        ...
        <el-form-item label="车间">
          <el-select v-model="searchInfo.workshopId" placeholder="请选择车间" filterable @change="getProductLine"
        ...
        <el-form-item label="线别">
          <el-select v-model="searchInfo.lineName" filterable placeholder="请选择线别" :loading="loading.selectLine"
                     class="input-item" clearable>
            <el-option v-for="item in option.productLineList" :key="item.id" :label="item.line" :value="item.line"></el-option>
          </el-select>
        </el-form-item>

        <!--<el-form-item label="机位丝锭数">-->
          <!--<el-input-number v-model="searchInfo.silkNum" placeholder="请输入机位丝锭数" :min="1" :max="99999999" class="input-item-16"></el-input-number>-->
        <!--</el-form-item>-->

        <el-form-item label="落桶方式">
          <el-select v-model="searchInfo.doffType" placeholder="请选择落桶方式" clearable>
            <el-option v-for="item in doffTypes" :key="item.id" :label="item.name" :value="item.value" class="input-item"></el-option>
          </el-select>
        </el-form-item>
      ...
      <el-table :data="tableData" border style="width: 100%" v-loading="loading.table">
        ...
        <el-table-column prop="startLine" label="开始线别"></el-table-column>
        <el-table-column prop="endLine" label="结束线别"></el-table-column>
        <el-table-column prop="silkNum" label="机位丝锭数"></el-table-column>
        <el-table-column prop="silkcarNum" label="丝车数量"></el-table-column>
        <el-table-column prop="silkcarSpecDesc" label="丝车规格"></el-table-column>
        <el-table-column label="落桶方式" min-width="100">
      ...
      <!-- 新增落桶绑定规则 -->
      <dialog-drop-rule ref="refDialogAdd" :shopList="shopList" :doffTypes="doffTypes"
                        :silkcarSpecList="silkcarSpecList" @submitSuccess="getData"></dialog-drop-rule>
    </div>
  </div>
</template>
...
<script>
  ...
  export default {
    components: {
      'dialog-drop-rule': require('./dialog-drop-rule.vue')
    },
    ...
    methods: {
      getData () {
        ...
        let param = {
          ...
          // silkNum: this.searchInfo.silkNum,
          lineName: this.searchInfo.lineName,
