<template>
  <div class="measure-printing">
    ...
    <el-table :data="tableData" @selection-change="handleSelectionChange" border v-loading="loading.table"
              style="width: 100%">
      <el-table-column type="selection" width="55"></el-table-column>
      <el-table-column prop="singleCode" label="编号" width="270"></el-table-column>
      ...
      <el-table-column prop="kindName" label="种类"></el-table-column>
      <el-table-column prop="batchNo" label="批号"></el-table-column>
      <el-table-column prop="silkSpec" label="规格" width="180"></el-table-column>
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          <el-button type="text" @click="btnMetering(scope.row)">计量</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
    ...
    <!--新增码单-->
    <dialog-create-barcode ref="refDialogCreateCode" :workShop="options.workshop" :lines="options.line"
                           :productName="options.productName" :grade="options.grade" :classes="options.classes"
                           :batcheItems="restaurants" :allKind="options.kind" @submitSuccess="getData">
...
<script>
  ...
  export default {
    components: {
      ...
      'dialog-poy-fdy-print': require('../measure-printing/dialog-print.vue')
    },
    data () {
      return {
        options: {
    ...
    methods: {
      getData () {
        ...
        api.automatic.barCode.getBoxCodeListForStorage(params)
        .then(response => {
      ...
      batchPrint () {
        ...
        if (this.multipleSelection.some(item => { return item.storageCode !== first.storageCode })) {
          return this.$message('请选择相同库位打印')
        }
        if (this.multipleSelection.some(item => { return item.productTypeName !== first.productTypeName })) {
          return this.$message('请选择相同产品打印')
        }
        let idArr = this.multipleSelection.map(item => { return item.id })
        api.automatic.barCode.printBoxCodeListForStorage({id: idArr.join(',')}).then(response => {
          let data = response.data
          if (data.messageType === 1) {
            if (selection[0].productTypeName.includes('短纤')) {
              this.product = 'dialog-staple-fiber-print'
            } else if (selection[0].productTypeName.includes('切片')) {
              this.product = 'dialog-slice-print'
            } else if (['poy', 'fdy'].some(item => { return selection[0].productTypeName.toLowerCase().includes(item) })) {
