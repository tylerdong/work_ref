  ...
  export default {
    ...
    methods: {
      ...
      btnPrint (item) {
        ...
        api.automatic.barCode.printBoxCodeForStorage(params).then(response => {
          let data = response.data
          if (response.data.messageType === 1) {
            if (data.data.productTypeName.includes('短纤')) {
              this.product = 'dialog-staple-fiber-print'
            } else if (data.data.productTypeName.includes('切片')) {
