    ...
    <div class="all-wrapper">
      <div class="action-bar">
        <el-form :model="search" :rules="rules" ref="form" :inline="true">
          <el-form-item>
            <el-input placeholder="请输入丝车编码" v-model="search.carCode"></el-input>
          </el-form-item>
      ...
      <div class="main-wrappper">
        <ul v-loading="loading.list">
          ...
          <li class="list-item" v-for="(item, index) in tableData">
            <div class="line">
              <div class="line1-item">丝车编码：</div>
              <div class="line1-item2 font-bold">{{item.silkCarNumber}}</div>
              <div class="line1-item">完成步骤：</div>
              <div class="line1-item2">{{item.productionProcessName}}-{{item.operateTypeName}}</div>
                  ...
                  <div class="dye-item" v-for="(item2, index) in item.silkInfoBoList">
                    ...
                    <div class="daye-itme-info">
                      <div v-if="item2.silkCode" :class="{red: item2.exceptionStatus}"
                           class="daye-itme-info-btn hand" @click="detailClick(item2, item.silkcarFlowId)">
                      </div>
                    </div>
                  </div>
                </div>
  ...
  export default {
    ...
    data () {
      return {
        ...
        rules: {
          carCode: [{ type: 'string', required: true, message: '请输入丝车编码' }],
    ...
    methods: {
      ...
      getData () {
        ...
        if (!this.search.carCode.trim()) {
          this.$message({type: 'warning', message: '请输入丝车编码'})
          return
        }
      ...
      detailClick (nowItem, silkcarFlowId) {
        this.$refs.waitDialog.show(nowItem, this.tableData, silkcarFlowId)
      },
      /* 分页 */
      pageSizeChange (size) {
        this.page.size = size
