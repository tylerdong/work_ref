...
<template>
  <div>
    <breadcrumb nameId="020403"></breadcrumb>
    <div class="hy-admin__main-container">
      ...
      <template>
        <el-table :data="tableData" border style="width: 100%" v-loading.body="loading">
          ...
          <el-table-column prop="libraryPlanBatchno" label="批号" min-width="200"></el-table-column>
          <el-table-column prop="libraryPlanNum" label="数量"></el-table-column>
  ...
  export default {
    ...
    methods: {
      getData () {
        ...
        api.automatic.collect.getLibraryPlanList(params).then(response => {
      ...
      add () {
        let params = {
          libraryPlanBatchno: this.dialogData.libraryPlanBatchno,
          libraryPlanNum: this.dialogData.libraryPlanNum
        }

        api.automatic.collect.addLibraryPlan(params).then(response => {
          const data = response.data
          if (data.messageType === 1) {
            this.$message({
              type: 'success',
      ...
      deleteFun (scope) {
        let params = {libraryPlanId: scope.row.libraryPlanId}
        this.delFun(params)
      },
      modifyItem (scope) {
        this.dialogData.libraryPlanId = scope.row.libraryPlanId
        this.dialogData.libraryPlanBatchno = scope.row.libraryPlanBatchno
        this.dialogData.libraryPlanNum = scope.row.libraryPlanNum
        this.$refs.refDialog.dialogFormVisible = true
      },
      modify () {
        let params = {
          libraryPlanId: this.dialogData.libraryPlanId,
          libraryPlanBatchno: this.dialogData.libraryPlanBatchno,
          libraryPlanNum: this.dialogData.libraryPlanNum
        }

        api.automatic.collect.updateLibraryPlan(params).then(response => {
          const data = response.data
          if (data.messageType === 1) {
            this.$message({
              type: 'success',
      ...
      delFun (params) {
        this.$confirm('是否确定删除用户', '提示', {
          ...
          api.automatic.collect.deleteLibraryPlan(params).then(response => {
            const data = response.data
            if (data.messageType === 1) {
              this.$message({
                type: 'success',
    ...
    data () {
      return {
        tableData: [],
        dialogData: {libraryPlanId: '', libraryPlanBatchno: '', libraryPlanNum: ''},
        input: '',
        loading: true,
        type: '',
        page: {
