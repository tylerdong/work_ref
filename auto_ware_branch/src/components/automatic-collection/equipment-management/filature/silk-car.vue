  ...
  <div>
    ...
    <div class="hy-admin__main-container">
      <div class="hy-admin__search-main cf">
        ...
        <el-input class="width1" v-model="searchInfo.numberStart" placeholder="请输入起始丝车号" ></el-input>
        <el-input class="width1" v-model="searchInfo.numberEnd" placeholder="请输入结束丝车号" ></el-input>
      ...
      <template>
        <el-table :data="list.tableData" border style="width: 100%" v-loading="loading.table"
          ...
          <el-table-column prop="number" label="丝车号" min-width="100"></el-table-column>
          <el-table-column prop="code" label="丝车编码" min-width="100"></el-table-column>
          ...
          <el-table-column label="操作" min-width="100">
            <template slot-scope="scope">
              <el-button @click="btnEdit(scope.row)" type="text">修改</el-button>
            </template>
          </el-table-column>
        </el-table>
      </template>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getSilkcarSpec () {
        api.automatic.device.getSilkcarSpecNoPage({desc: ''}).then((response) => {
          ...
          if (data.messageType === 1 && data.data.length > 0) {
            this.list.specificationList = data.data
          }
        }).catch((e) => {
          console.log(e)
        }).finally(() => {})
      ...
      /* 添加 */
      btnAdd () {
        this.$refs.refDialogAdd.btnOpen({type: '新增'})
      },

      /* 修改 */
      btnEdit (data) {
        console.log(data)
        this.$refs.refDialogEdit.btnOpen({...data, type: '修改'})
      },
