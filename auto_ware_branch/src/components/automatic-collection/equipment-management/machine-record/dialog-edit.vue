  ...
  <dialog-side title="修改" width="450px" :visible.sync="dialog.visible">
    <el-form :model="form" :rules="rules" ref="form" label-width="100px">
      ...
      <el-form-item label="卷绕头数" prop="partNum">
        <el-input-number v-model="form.partNum" placeholder="请输入卷绕头数" :min="2"
                         :max="99999999" @change="partNumChage"></el-input-number>
      </el-form-item>
      <el-form-item label="锭数" prop="spinNum">
        <el-input-number v-model="form.spinNum" placeholder="请输入卷绕头数" :min="1"
  ...
  export default {
    ...
    methods: {
      ...
      btnOpen (row) {
        ...
        this.selectChangeShop(row.workshopId)
        this.form.id = row.id
        ...
        api.automatic.productPlan.getAllLine({
          ...
          if (data.messageType === 1) {
            for (let item of data.data) {
              this.list.productLineList.push({id: item.id, name: item.line})
            }
          }
        })
      },
