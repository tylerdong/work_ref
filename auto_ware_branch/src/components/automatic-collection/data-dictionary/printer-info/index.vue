...
<template>
  <div>
    <!--<main-breadcrumb nameId="020409"></main-breadcrumb>-->
    <div class="hy-admin__main-container">
      ...
      <el-table :data="tableData" border v-loading="loading.list" element-loading-text="拼命加载中">
        ...
        <el-table-column prop="workshopName" label="车间" show-overflow-tooltip></el-table-column>
  ...
  export default {
    ...
    methods: {
      getData () {
        ...
        api.automatic.dictionary.getPrintList(params).then((response) => {
          ...
          if (data.messageType === 1) {
            ...
            for (let item of data.data.list) {
              if (item.type === '1') {
                item.typeName = '丝锭条码打印'
              }
              if (item.type === '2') {
                item.typeName = '丝车条码打印'
              }
              if (item.type === '3') {
                item.typeName = '包装二维码'
              }
