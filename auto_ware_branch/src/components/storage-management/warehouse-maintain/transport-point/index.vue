    ...
    <div class="hy-admin__main-container">
      ...
      <template>
        <el-table :data="tableData" border style="width: 100%" v-loading="loading.table">
          ...
          <el-table-column prop="name" label="装运点名称"></el-table-column>
          <el-table-column prop="description" label="描述"></el-table-column>
          <el-table-column label="操作" min-width="100">
            <template slot-scope="scope">
              <el-button type="primary" @click="btnEdit(scope.row)">修改</el-button>
              <el-button type="primary" @click="btnDelete(scope.row)">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
      </template>
  ...
  export default {
    ...
    methods: {
      getData () {
        ...
        api.storage.warehouseMaintain.getTransportPointList({
        ...
        this.$confirm('是否确认删除?', '提示', {
          ...
          beforeClose: (action, instance, done) => {
            if (action === 'confirm') {
              ...
              api.storage.warehouseMaintain.deleteTransportPoint({
