  ...
  <div class="page-wrapper">
    ...
    <el-table :data="tableData" border v-loading="loading.table">
      ...
      <el-table-column width="276" label="操作">
        <template slot-scope="scope">
          <el-button v-if="scope.row.status === 'PENDING'" @click="supplementClick(scope.row)" type="text">出货安排</el-button>
          <!--<el-button v-if="scope.row.status === 'PENDING'" @click="bfOut(scope.row)" type="text" size="small">提前出库</el-button>-->
          <el-button v-if="scope.row.status !== 'PENDING'" @click="detailClick(scope.row)" type="text">查看详情</el-button>
          <el-button v-if="scope.row.status === 'PROCESSED'" @click="cancel(scope.row)" type="text">取消调拨</el-button>
          <el-button v-if="scope.row.status === 'PROCESSED' || scope.row.status === 'CHECKING' || scope.row.status === 'PICKUP_FAILED'" @click="manualPickup(scope.row)" type="text">手动拣配</el-button>
          <el-button v-if="scope.row.status === 'CHECKING' || scope.row.status === 'CHECKED'" @click="cancelPickup(scope.row)" type="text">取消拣配</el-button>
          <el-button v-if="scope.row.status === 'SAP_FINISH'" @click="cancelPost(scope.row)" type="text">取消过账</el-button>
          <el-button v-if="scope.row.status === 'SAP_FINISH'" @click="transfer(scope.row)" type="text">过账转移</el-button>
        </template>
      </el-table-column>
    </el-table>
