      ...
      <div v-loading="loading.dialog">
        <el-row :gutter="20">
          ...
          <el-col :span="6">
            ...
            <el-col class="line-height36" :span="16">
              <select-plate-number :disabled="plateNumberDisable" :plateNumber="nowData.plateNumber" @plateNumberChange="plateNumberChange"></select-plate-number>
            </el-col>
          </el-col>
          <el-col :span="6">
                ...
                <el-tab-pane label="包装申请（整箱）" name="2" :disabled="outer.saleRequisitionDetailBoList.level !== 'AA'">
                  ...
                  <el-table :data="outer.requestPackageList" class="x-scroll">
                    ...
                    <el-table-column label="生产线（锭）">
                      <template slot-scope="scope">
                        <el-input-number @change="getCurrentWeight(outer)" v-model="scope.row.productLineCount" :controls="false"></el-input-number>
                      </template>
                    </el-table-column>
                    <el-table-column label="锭重">
                      <template slot-scope="scope">
                        <el-select @change="getCurrentWeight(outer)" v-model="scope.row.silkWeight" placeholder="请选择" :disabled="outer.saleRequisitionDetailBoList.level !== 'AA'">
                          <el-option v-for="item in options.silkWeight" :key="item" :label="item" :value="item"></el-option>
                        </el-select>
                      </template>
                    </el-table-column>
                    <el-table-column label="箱数">
                      <template slot-scope="scope">
                        <el-input-number @change="getCurrentWeight(outer)" v-model="scope.row.boxCount" :controls="false"></el-input-number>
                      </template>
                    </el-table-column>
...
<script>
  ...
  export default {
    components: {
      ...
      'select-plate-number': require('common/select-plate-number.vue'),
    ...
    methods: {
        ...
        api.storage.warehouseMaintain.getListByType({type: 'WIRE'}).then(response => {
          ...
          if (data.messageType === 1) {
            if (data.data.length > 0) {
              this.options.plateNumber = data.data
            }
          } else {
      ...
      show (data) {
        console.log(data)
        this.plateNumberDisable = data.plateNumber !== ''
        console.log(this.plateNumberDisable)
        this.loadingPointId = ''
        this.nowData = this.deepClone(data)
        this.title = '出货安排'
