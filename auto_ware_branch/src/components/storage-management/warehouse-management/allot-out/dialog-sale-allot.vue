<template>
  <div>
    <el-dialog :title="title" width="90%" :visible.sync="dialogVisible">
      <div v-loading="loading.dialog">
        <el-row :gutter="0">
          <el-col :span="5">
            <el-col :span="9" class="label1">发货日期</el-col>
            <el-col class="line-height36" :span="15">
          ...
          <el-col :span="4">
            <el-col :span="8" class="label1">共享托盘</el-col>
            <el-col class="line-height36" :span="16">
              <el-checkbox v-model="isUseSharedPallet" true-label="Y" false-label="N"></el-checkbox>
            </el-col>
          </el-col>
          <el-col :span="6">
              ...
              <el-tabs v-model="outer.activeTab" type="border-card">
                <!--<el-tab-pane label="仓库出货" name="1">-->
                  <!--<el-table :data="outer.displayStockNumBoList" class="x-scroll">-->
                    ...
                    <!--&lt;!&ndash;<el-table-column label="生产日期">&ndash;&gt;-->
                      <!--&lt;!&ndash;<template slot-scope="scope">{{scope.row.productTime | timeFormat('YYYY-MM-DD')}}</template>&ndash;&gt;-->
                    <!--&lt;!&ndash;</el-table-column>&ndash;&gt;-->
                    <!--&lt;!&ndash;<el-table-column prop="storageName" label="库位"></el-table-column>&ndash;&gt;-->
                    <!--<el-table-column prop="storageNum" label="库存箱数"></el-table-column>-->
                    <!--<el-table-column label="发货箱数">-->
                      <!--<template slot-scope="scope">-->
                        <!--<el-input-number @change="getCurrentWeight(outer)" class="width60" v-model="scope.row.outStockNum" :controls="false"></el-input-number>-->
                      <!--</template>-->
                    <!--</el-table-column>-->
                    <!--<el-table-column label="拆包箱数">-->
                      <!--<template slot-scope="scope">-->
                        <!--<el-input-number :disabled="!(outer.saleRequisitionDetailBoList.level === 'AA' && scope.row.productType === 'INNER_SALE')"-->
                                         <!--class="width60" v-model="scope.row.devanningNum" :controls="false"></el-input-number>-->
                      <!--</template>-->
                    <!--</el-table-column>-->
                  <!--</el-table>-->
                <!--</el-tab-pane>-->
                <el-tab-pane label="包装申请（整箱）" name="2" class="x-scroll" :disabled="outer.saleRequisitionDetailBoList.level !== 'AA'">
                  <div class="cf">
                  ...
                  <el-table :data="outer.requestPackageList" class="x-scroll">
                    <el-table-column label="拆包区（锭）">
                      <template slot-scope="scope">
                        <el-input-number @change="getCurrentWeight(outer)" class="width60" v-model="scope.row.unCrateCount" :controls="false"></el-input-number>
                      </template>
                    </el-table-column>
                    <el-table-column label="生产线（锭）">
                      <template slot-scope="scope">
                        <el-input-number @change="getCurrentWeight(outer)" class="width60" v-model="scope.row.productLineCount" :controls="false"></el-input-number>
                      </template>
                    </el-table-column>
                    <el-table-column label="锭重">
                      <template slot-scope="scope">
                        <el-select @change="getCurrentWeight(outer)" v-model="scope.row.silkWeight" placeholder="请选择" :disabled="outer.saleRequisitionDetailBoList.level !== 'AA'">
                    ...
                    <el-table-column label="箱数">
                      <template slot-scope="scope">
                        <el-input-number @change="getCurrentWeight(outer)" class="width60" v-model="scope.row.boxCount" :controls="false"></el-input-number>
                      </template>
                    </el-table-column>
                    ...
                    <el-table-column label="">
                      <template slot-scope="scope">
                        <el-button type="primary" @click="deleteClick(outer, scope.$index)">删除</el-button>
                      </template>
                    </el-table-column>
                  </el-table>
                </el-tab-pane>
  ...
  export default {
    ...
    methods: {
      ...
      show (data) {
        ...
        this.isUseSharedPallet = 'N'
        this.dialogVisible = true
        this.loading.dialog = true
        api.storage.warehouseManagement.getUnDoSaleRequisitionById({
          ...
          if (data) {
            ...
            for (let outer of data) {
              ...
              outer.saleRequisitionDetailBoList.currentWeight = 0
              for (let item2 of outer.stockNumBoList) {
                item2.outStockNum = 0
                item2.devanningNum = 0
              }
              outer.displayStockNumBoList = JSON.parse(JSON.stringify(outer.stockNumBoList))
              outer.requestPackageList = []
            }
            this.formData = data
      ...
      getPrintData (data) {
        ...
        for (let outer of data) {
          ...
          for (let titleBo of outer.titleBos) {
            ...
            item.isUseSharedPallet = this.isUseSharedPallet
      ...
      commitRequisition () {
        ...
        if (!this.loadingPointId) {
          errorMessage.push('装运点不能为空')
        }
        let validData = this.formData.find(item => {
          return item.saleRequisitionDetailBoList.currentWeight > item.saleRequisitionDetailBoList.netWeight
        })
        if (validData) {
          errorMessage.push(`当前重量大于净重，物料号${validData.saleRequisitionDetailBoList.material}`)
        }
        let params = {
          ...
          plateNumber: this.nowData.plateNumber,
          loadingPointId: this.loadingPointId,
          useSharedPallet: this.isUseSharedPallet,
        ...
        for (let item of this.formData) {
          ...
          for (let item3 of item.requestPackageList) {
            ...
            if (!item3.netWeight && saleRd.level === 'AA') { errorMessage.push('锭重不能为空') }
            if (!item3.boxCount) { errorMessage.push('箱数不能为空') }
