<template>
  <el-dialog
    class="dialog-role-manager-deploy"
    width="50%"
    :title="dialogInfo.title"
    :visible.synv="dialogInfo.toggle"
    @close="close">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      close () {
        this.$emit('callback')
        this.dialogInfo.toggle = false
      },

      getData () {
        api.marManager.getListPrivilegeRoleMap({
          roleId: this.dialogInfo.id
        }).then(response => {
      ...
      handleAdd () {
        if (this.tempSelect.noList.length) {
          ...
          for (let temp of this.tempSelect.noList) {
            params.privilegeIdList.push({
              privilegeId: temp.id
            })
          }

          api.marManager.addPrivilegeRoleMap(params).then(response => {
            const data = response.data
            console.log(1)
            if (data.messageType === 1) {
              console.log(2)
      ...
      handleRemove () {
        if (this.tempSelect.addList.length) {
          ...
          api.marManager.deletePrivilegeRoleMapByPriId(params).then(response => {
            const data = response.data
            if (data.messageType === 1) {
