  ...
  <div class="shortVideo">
    ...
    <div class="mb-20 clearfix">
      ...
      <Select v-model="search.showSource" placeholder="请选择来源" transfer=transfer clearable style="width:150px;">
        <Option v-for="(item) in option.source" :value="`${item.source}+${item.accessMethod}`" :source="item.source" :accessMethod="item.accessMethod" :key="item.accessMethod">{{ item.showSource }}</Option>
      </Select>
      标题搜索：
    ...
    >
      ...
      >
        ...
        <FormItem label="视频作者:" prop="auth">
          <Select v-model="form.auth"  style="width:200px">
        ...
        <FormItem label="视频播放:" >
          ...
          <video style="width:200px;height:300px;" :src="url" controls v-if="form.status!='UPLOAD_PENDING'"></video>
        </FormItem>
...
import {getshortList,getVideoSourceList,auditVideo,unloadVideo,getUserList} from '@/api/shortVideo'
...
export default {
    ...
    return {
      ...
      table: {
        ...
        columns: [{ title: "序号", width: 100, key: "id", align: "center" },
          ...
          { title: "更新时间", width: 200, key: "gmtModified", align: "center" ,render: (h,params)=>{
              return h('div',
                  dateFns.format(params.row.gmtModified, 'YYYY-MM-DD HH:mm')
              )
          }},
          { title: "创建人", width: 100, key: "creator", align: "center" },
          { title: "操作", width: 150, fixed: 'right', align: "center",render: (h, params) => {
            ...
            const audit = h('Button', {
                ...
                on: {
                    click: () => {
                      console.log( '审核',params.row.status);
                      this.modalshow=true;
                      this.form = JSON.parse(JSON.stringify(params.row))
                      this.url = params.row.videoUrl
            ...
            if(status=='CHECK_PENDING'||status=='FIRST_AUDIT_FINISH'){
              return h('div', [
                audit
              ]);
  ...
  methods:{
    ...
    getData(){
      ...
      console.log(this.search.timeSpan);
