;(function() {
    ...
    var diceIconHtml = "<i class='dh-icon dh-icon-dice'>开始抽奖</i>"
    var saveIconHtml =
        "<i class='dh-icon dh-icon-dice'>💾<svg><use xlink:href='#dh-save'/></svg></i>"
    var okayIconHtml = "<i class='dh-icon dh-icon-okay'>停止</i>"
    var crownIconHtml = "<i class='dh-icon dh-icon-crown'></i>"

    // 生成奖品视图
    function awardsHtml(awards) {
        let htmlStr = ``
        awards.forEach(ele => {
            htmlStr += `<dl class="big-c">
                      <dt class="big-t">${ele.cateName}</dt>
                        <dd class="big-d">`
            ele.item.forEach(subItem => {
                htmlStr += `<dl class="s-c">
      <dt class="s-t">${subItem.name}</dt>
      <dd class="s-d"><ul>`
                subItem.item.forEach(subsubItem => {
                    htmlStr += `<li>`
                    if (subsubItem.isExtract) {
                        htmlStr += `✅ <span class="mr-20">${
                            subsubItem.awards
                        }*${subsubItem.count}</span>`
                    } else {
                        htmlStr += `<a href="javascript:;" class="mr-20 J-rock" data-title="${
                            subsubItem.awards
                        }" data-count="${subsubItem.count}">🎲 ${
                            subsubItem.awards
                        }*${subsubItem.count}</a>`
                    }
                    htmlStr += `<a href="javascript:;" data-pic="${
                        subsubItem.image
                    }" class='J-show-pic'>🎁</a></li>`
                })
                htmlStr += `</ul></dd>`
            })
            htmlStr += `</dd>
    ...
    // 更新奖品抽取状态
    function updateAwardsStatus(name, awards) {
        awards.forEach(ele => {
            ele.item.forEach(sele => {
                sele.item.forEach(ssele => {
                    if (ssele.awards == name) {
                        ssele.isExtract = true
                    }
                })
            })
        })
    ...
    // 初始化奖品
    function initAwards() {
        if (confirm('所有已经开奖的记录将重置，请谨慎操作？') == true) {
            localStorage.setItem('awards', JSON.stringify(awards))
            $('#dh-awards').removeClass('is-active')
        }
    }

    // 获取剩余未中奖人数
    function getUnAwardsPersonCount() {
        // 还剩未中奖人数
        var lastPersons = settings.data.length
        var hasAwardsPersons = 0
        if (localStorage.getItem('lotteryHistory')) {
            var lh = JSON.parse(localStorage.getItem('lotteryHistory'))
            for (let index = 0; index < lh.length; index++) {
                hasAwardsPersons = hasAwardsPersons + parseInt(lh[index].count)
            }
        }
        lastPersons = settings.data.length - hasAwardsPersons
        return lastPersons
    }
    ...
    "
        ...
        var btn = $(
            "\
      <div class='btn-bar'>\
        <span class='persons'>抽奖人数：<input type='number' class='persons-input' id='lottery-person-count' />人,余<span class='unawp' id='total-persons'></span>人</span>\
        <a href='javascript:;' class='btn-go' id='dh-lottery-go'>开始抽奖</a>\
        <a href='javascript:;' class='btn-list' id='dh-history-show' style='margin-right:10px'>中奖名单</a>\
        <a href='javascript:;' class='btn-list' id='dh-awards-show'>选择奖品</a>\
      </div>\
    "
        )
        //中奖用户展示弹框
        var modal = $(
            "\
      <div class='dh-animated dh-modal" +
                (isAppleOs ? ' is-mac' : '') +
                "' id='dh-lottery-winner'>\
    ...
    "
        ...
        // 奖品弹窗
        var awardModal = $(`
      <div class='dh-animated dh-modal J-awards' id='dh-awards'>
        <div class='dh-modal-background'></div>\
        <div class='dh-modal-content'>\
        </div>\
        <button class='dh-modal-close'></button>\
        <button class='dh-history-clean dh-modal-btn' id="init-award">初始化奖品</button>\
      </div>
    `)
        ...
        //     </div>\
        //     <button class='dh-modal-close'></button>\
        //     <button class='dh-history-clean dh-modal-btn'>清除中奖历史</button>\
    ...
    "
        ...
        $('#lottery-person-count').val(settings.number)
        // 修改抽奖人数
        $('#lottery-person-count').change(function() {
            var count = parseInt($(this).val()) ? parseInt($(this).val()) : 1
            // $(this).val(count)
        ...
        // 显示奖品modal
        $('#dh-awards-show').click(function() {
            localStorage.removeItem('lotteryNow')
            showAwards()
            // $('#dh-lottery-history .dh-number').text(settings.number);
        })
        $('#dh-awards .dh-modal-close').click(function() {
            return $('#dh-awards').removeClass('is-active')
        })
        $('#dh-awards .dh-history-clean').click(function() {
            initAwards()
        })
        $('#dh-awards').on('click', '.J-rock', function() {
            const $this = $(this)
            ...
            // 设置当前抽奖名称
            localStorage.setItem('lotteryNow', title)
        ...
        // 显示礼物图片
        $('#dh-awards').on('click', '.J-show-pic', function() {
            const $this = $(this)
            const pic = $this.data('pic')
            const awardsImgBox = $(
                `<div class="imb-box-modal"><div class="img-box"><div class="ibb"><img src='${pic}' /></div></div></div>`
            )
            $('body').append(awardsImgBox)
        })
        // 隐藏礼物图片
        $('body').on('click', '.imb-box-modal', function() {
            $('.imb-box-modal').remove()
        })
    ...
    var appendNewUserToList = function(item) {
        var html =
            `
      <div class="column">
        <div class="profile" data-profile="${encodeURIComponent(
            JSON.stringify(item)
        )}">
          <div class="profile__parent">
            ...
            (function() {
                if (item.avatar) {
                    return `<div class="avatar"><span class="image avatar-image is-128x128"><img src="${
                        item.avatar
                    }" alt="avatar" /></span></div`
                } else {
                    return `<div class="avatar"><span class="image dh-name-avatar avatar-image is-128x128">${item
                        .data[settings.title] || item.name}</span></div>`
                }
            })() +
            `</div>
              </div>
    ...
    var readyLottery = function() {
        ...
        if (localStorage.getItem('lotteryHistory'))
            settings.winnerHistory = JSON.parse(
        ...
        new MaterialAvatar(
            document.getElementsByClassName('dh-name-avatar'),
            avatarOptions
        )
        console.log('Lottery: ' + settings.data.length + ' player')
        $('#total-persons').text(getUnAwardsPersonCount())
    ...
    `)
        ...
        if (winnerProfile) {
            if (
                winnerProfile['data'] &&
                Object.keys(winnerProfile['data']).length > 0
            ) {
            ...
            if (winnerProfile['avatar']) {
                el.find('.avatar-image img').attr(
                    'src',
                    winnerProfile['avatar']
                )
            } else {
                el.find('.avatar-image img').remove()
                el.find('.avatar-image')
                    .addClass('dh-name-avatar')
                    .text(profileName)
            }
    ...
    var moveToTarget = function(i, target) {
        ...
        move('#dh-lottery-selector #selector_' + i)
            .x(positionList[target].left - 4)
            .y(positionList[target].top - 4)
            .ease('in-out')
            .duration(200)
            .end()
    ...
    var lotteryOnce = function(selector = 0) {
        ...
        if (
            (settings.once && settings.winnerList[targetIndex]) ||
            $.inArray(targetIndex, currentTarget) >= 0
        ) {
    ...
    var stopLottery = function() {
        ...
        var curAward = localStorage.getItem('lotteryNow')
        var awardCount = localStorage.getItem('lotteryConfigNumber')
        var awards = $(`<h1 class='winner-t'>${curAward} * ${awardCount}</h1>`)
        $('#dh-lottery-winner .dh-modal-content').append(awards)

        // 更新本轮中奖者信息
        for (var i = 0; i < currentTarget.length; i++) {
            var winnerProfile = JSON.parse(
                decodeURIComponent(
                    $($('.profile')[currentTarget[i]]).data('profile')
                )
            )
        ...
        // 根据中奖者人数调整双栏布局和文字大小
        $('#dh-lottery-winner .dh-modal-content')
            .removeClass('dh-morewinner')
            .removeClass('dh-solowinner')

        $('.dh-modal-content .profile-item').css('font-size', '50px')
        $('.dh-modal-content .profile-name').css('line-height', '80px')

        if (currentTarget.length === 1) {
            $('.dh-modal-content .profile-item').css('font-size', '90px')
            $('.dh-modal-content .profile-name').css('line-height', '90px')
            $('#dh-lottery-winner .dh-modal-content').removeClass(
                'dh-morewinner'
            )
            $('#dh-lottery-winner .dh-modal-content').addClass('dh-solowinner')
        } else if (currentTarget.length > 1 && currentTarget.length < 6) {
            $('#dh-lottery-winner .dh-modal-content').removeClass(
                'dh-morewinner'
            )
            $('#dh-lottery-winner .dh-modal-content').removeClass(
            ...
            $('.dh-modal-content .profile-item').css('font-size', '70px')
            $('.dh-modal-content .profile-name').css('line-height', '180px')
        } else if (currentTarget.length >= 6) {
            $('#dh-lottery-winner .dh-modal-content').removeClass(
                'dh-solowinner'
            )
            $('#dh-lottery-winner .dh-modal-content').addClass('dh-morewinner')
        }
        ...
        history.awards = localStorage.getItem('lotteryNow')
        history.count = parseInt(localStorage.getItem('lotteryConfigNumber'))
        // 把获奖名单的数组转对象
        history.winner = {}
        ...
        localStorage.setItem(
            'lotteryHistory',
            JSON.stringify(settings.winnerHistory)
        )

        // 显示中奖名单，奖品按钮
        $('#dh-history-show').show()
        $('#dh-awards-show').show()

        // 更新奖品列表
        const awardsArr = JSON.parse(localStorage.getItem('awards'))
        const newAwardsArr = updateAwardsStatus(
            localStorage.getItem('lotteryNow'),
            awardsArr
        )
        localStorage.setItem('awards', JSON.stringify(newAwardsArr))
    ...
    var startLottery = function() {
        ...
        if (settings.once && getUnAwardsPersonCount() < settings.number) {
        ...
        // 没有选择奖项时，提示
        if (!localStorage.getItem('lotteryNow')) {
            // alert('No user left to participate in lottery.');
            alert('请先选择要抽的奖项.')
            return false
        }
        ...
        if (settings.timeout)
            lotteryTimeout = setTimeout(stopLottery, settings.timeout * 1000)
        $('#dh-lottery-go')
            .removeClass('primary')
            .addClass('success')
            .html(okayIconHtml)
    ...
    var cleanHistory = function() {
        if (confirm('确定清除所有中奖历史吗，请谨慎操作?') == true) {
    ...
    function showAwards() {
        var htmlStr = awardsHtml(JSON.parse(localStorage.getItem('awards')))
        $('#dh-awards .dh-modal-content').html(htmlStr)
        $('#dh-awards').addClass('is-active')
    }

    var showHistory = function() {
        var tplItem = function(data) {
            return `
      <div class='dh-history-item'>
        <div class='dh-history-info'>
          <h1>${data.awards}*${data.count}</h1>
          <p>${data.time}</p>
        </div>
        ...
        ` +
                (function() {
                    if (data.avatar) {
                        ...
                        return `<div class="avatar"><span class="image dh-name-avatar avatar-image is-128x128">${data
                            .data[settings.title] || data.name}</span></div>`
                    }
                })() +
                `
    ...
    var controller = {
        ...
        init: function(options) {
            ...
            if (!options.number && localStorage.lotteryConfigNumber) {
                settings.number =
                    parseInt(localStorage.lotteryConfigNumber) || 1
            }
        },
        // 抽奖
        start: function() {
    ...
    $.lottery = function(method) {
        if (controller[method]) {
            return controller[method].apply(
                this,
