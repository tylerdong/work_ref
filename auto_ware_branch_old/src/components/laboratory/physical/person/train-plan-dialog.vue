<template>
  <el-dialog :title="title" :visible.sync="dialogVisible" width="80%" append-to-body modal-append-to-body>
    <el-form :model="form" :rules="rules" ref="form" label-width="108px">
      ...
      <el-form-item label="讲师" prop="lecturer">
        <el-select v-model="form.lecturer" placeholder="请选择讲师" :disabled="formData.type === 'view'"
          ...
          <el-option v-for="(item,index) in option.users" :label="item.useName" :key="index" :value="item.id"></el-option>
        </el-select>
      </el-form-item>
  ...
  export default {
    ...
    methods: {
      ...
      show (data) {
        ...
        this.$nextTick(() => {
          ...
          if (['view', 'edit'].includes(data.type)) {
            api.physicalLaboratory.labTrainingPlanController.getLabTrainingPlanVoById({id: data.trainingPlanId}).then(response => {
              let data = response.data
              if (data.success) {
                data = data.data
                this.form.trainingTile = data.trainingTile
      ...
      add (params) {
        ...
        api.physicalLaboratory.labTrainingPlanController.createLabTrainingPlanVo(params).then(response => {
      ...
      edit (params) {
        this.loading.confirm = true
        api.physicalLaboratory.labTrainingPlanController.updateLabTrainingPlanVo(params).then(response => {
          if (response.data.success) {
            this.$emit('success')
            this.dialogVisible = false
          } else {
