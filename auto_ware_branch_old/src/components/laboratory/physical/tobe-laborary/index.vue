...
<template>
  <div>
    <div class="hy-admin__main-container">
      <div class="flex-div-row">
        ...
        <div class="flex-div-column hy-admin__search-main cf" ref="container" :style="classData">
          <div class="fr">
            <el-input class="search-input" placeholder="请输条码号" v-model="search.barCode"></el-input>
          ...
          <el-tabs v-model="activeName" type="card" @tab-click="handleClick" style="margin-top: 20px; width: 100%;">
            <el-tab-pane :name="item.id" v-for="(item, index) in tableTabs" key="index" :label="item.name" :key="index" style="background-color: #fff;">
            </el-tab-pane>
          </el-tabs>
          <el-table :data="tableData" border v-loading="loading.list" element-loading-text="拼命加载中" height="400">
            <el-table-column prop="labType" label="类型" width="70" fixed></el-table-column>
            <el-table-column prop="barCode" label="条码号" width="115" fixed></el-table-column>
            <el-table-column prop="batchNumber" label="批号" width="100" fixed></el-table-column>
            <el-table-column prop="spec" label="规格(dtex/f)" width="120" fixed></el-table-column>
            <el-table-column prop="productLine" label="产线" width="70" fixed></el-table-column>
            ...
            <el-table-column label="操作" width="300">
              <template slot-scope="scope">
                ...
                <el-button v-if="scope.row.fileId && scope.row.fileId !== ''" @click="previewExcel(scope.row.fileId)" type="text"
                           size="small">查看excel数据
                </el-button>
              </template>
            </el-table-column>
          </el-table>
    ...
    <dialog-do-experiment ref="refDoExperiment" @reloadData="getListData"></dialog-do-experiment>
    <dialog-excel-preview ref="excelPreivew"></dialog-excel-preview>
  </div>
</template>
<script>
  ...
  export default {
    components: {
      'dialog-do-experiment': require('./dialog-tobe-side.vue'),
      'dialog-excel-preview': require('./dialog-excel-preivew.vue')
    },
    created () {},
    data () {
      return {
        ...
        firstId: '',
        userInfo: '',
        excelPriviewDialog: false,
        fileCode: '',
        classData: { width: '', 'overflow-x': 'scroll' }
      }
    },
    props: {},
    filters: {
    ...
    methods: {
      ...
      previewExcel (fileId) {
        this.$refs.excelPreivew.show({fileId: fileId})
      }
    }
  }
</script>
