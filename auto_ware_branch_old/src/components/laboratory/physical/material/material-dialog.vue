  ...
  export default {
    ...
    methods: {
      ...
      add (params) {
        ...
        api.physicalLaboratory.labMaterialController.createLabMaterialDo(params).then(response => {
      ...
      update (params) {
        this.loading.confirm = true
        api.physicalLaboratory.labMaterialController.updateLabMaterialDo(params).then(response => {
          const data = response.data
          if (data.success === true) {
            this.dialogVisible = false
            this.$emit('success')
