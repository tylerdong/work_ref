<template>
  <el-dialog size="tiny" :title="title" :visible.sync="dialogVisible" width="80%">
    <el-form :rules="rules" ref="form" :model="form" label-width="108px">
      <el-form-item label="仪器名称" prop="groupId" required>
        <el-select class="repair-form-item" v-model="form.groupId" @change="changeGroupId" :disabled="this.type !== 'add'" filterable clearable>
      ...
      <el-form-item label="仪器编号" prop="instrumentId" required>
        <el-select v-loading="loading.instrument" class="repair-form-item" v-model="form.instrumentId" :disabled="this.type !== 'add'" filterable clearable>
          <el-option v-for="item in options.instrument" :key="item.id" :label="item.number" :value="item.id"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="事故原因" prop="accidentCause" required>
        <el-input class="repair-form-item" type="textarea" v-model="form.accidentCause" resize="none"></el-input>
      </el-form-item>
      <el-form-item label="报修人" prop="reporter" required>
        <el-select class="repair-form-item" v-model="form.reporter" filterable clearable>
          <el-option v-for="item in options.user" :key="item.id" :label="item.useName" :value="item.id"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="维修人" prop="repairer" required>
        <el-select class="repair-form-item" v-model="form.repairer" filterable clearable>
          <el-option v-for="item in options.user" :key="item.id" :label="item.useName" :value="item.id"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="维修项目" prop="aintenanceProject">
        <el-input type="textarea" class="repair-form-item" v-model="form.aintenanceProject" resize="none"></el-input>
      </el-form-item>
      <el-form-item label="登记人" prop="registerName">
        <el-input class="repair-form-item" v-model="form.registerName" disabled></el-input>
      </el-form-item>
      <el-form-item label="登记时间" prop="registerDate">
        <el-date-picker class="repair-form-item" type="datetime" v-model="form.registerDate" disabled></el-date-picker>
      </el-form-item>
    </el-form>
...
<script>
  ...
  export default {
    ...
    methods: {
      getUserList () {
        api.physicalLaboratory.userManagerCenter.normalUserList({
          pageIndex: 1,
          pageCount: 10000
        }).then(response => {
      ...
      changeGroupId (groupId) {
        ...
        api.physicalLaboratory.labInstrumentManagement.getLabInstrumentManagementDoList(params).then(response => {
      ...
      add (params) {
        api.physicalLaboratory.labInstrumentRepair.createLabInstrumentRepairDo(params).then(response => {
          const data = response.data
          if (data.success === true) {
...
<style>
  .repair-form-item.el-select .el-input{
    width: 100%;
  }
</style>
