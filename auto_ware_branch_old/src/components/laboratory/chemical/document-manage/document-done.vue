  ...
  export default {
    ...
    methods: {
      IsShare (data) {
        ...
        api.chemicalLaboratory.labFileController.changeDisplayLabFileDo(params).then((response) => {
      ...
      preview (data) {
        ...
        if (['xls', 'xlsx', 'pdf'].includes(data.fileType)) {
          ...
          api.chemicalLaboratory.labFileController.getLabFile(param).then((response) => {
      ...
      downLoad (data) {
        this.downloadHref = window.global.chemicalAjaxBaseUrl + 'api/lab/report/labFileController/downloadLabFile?id=' + data.id
        this.$nextTick(() => {
      ...
      delDocument (data) {
        this.$confirm('是否删除该文件, 是否继续?', '提示', {
          ...
          api.chemicalLaboratory.labFileController.delLabFile(param).then((response) => {
            let data = response.data
            if (data.success) {
      ...
      getAllFileByGroupId () {
        ...
        api.chemicalLaboratory.labFileController.getAllLabFileDos(param).then((response) => {
          let data = response.data
