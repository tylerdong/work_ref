<template>
  <el-dialog title="人员选择" :visible.sync="visible" width="80%">
  ...
  export default {
    ...
    methods: {
      show (scope, isModify) {
        ...
        this.getGroupList(scope.classesId, scope.schedulingEmployeeMapInfoBoList, scope.groupId)
      },
      close () {
        this.tableData.splice(0, this.tableData.length)
        this.visible = false
      ...
      getGroupList (value, listSelected, id) {
        ...
        api.automatic.person.getEmployeeListByGroupId(params).then(response => {
