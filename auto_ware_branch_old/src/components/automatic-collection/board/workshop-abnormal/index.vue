...
<script>
  ...
  // const abnormalPage = {pageIndex: 1, pageCount: 10, workshopId: '', line: ''}
  // const planPage = {pageIndex: 1, pageCount: 3, workshopId: '', lineId: ''}
  // const showRowlength = 5
  export default {
    components: {},
    data () {
      return {
    ...
    methods: {
      showClick () {
        ...
        if (this.search.workshopId && this.search.lineId) {
          this.timeinter = setInterval(this.searchclick, 5000)
        }
      },
      getWorkshopList () {
        ...
        $.ajax({
          ...
          success: (response) => {
            ...
            if (response.data.pageVo) {
              this.renderPageData(value.data, response.data.pageVo)
            }
