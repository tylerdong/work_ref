<template>
  <dialog-side width="380px" :title="title" :visible.sync="dialogFormVisible">
    <el-form :model="newInfo" :rules="rules" ref="newInfo" :label-width="formLabelWidth">
      <el-form-item label="降等原因" prop="name">
        <el-input v-model="newInfo.name" placeholder="请输入降等原因" :disabled="disabled"></el-input>
      </el-form-item>
      <el-form-item label="降等原因类别" prop="downGradeReasonTypeId">
        <el-select v-model="newInfo.downGradeReasonTypeId" placeholder="请选择降等原因类别" :disabled="disabled">
    ...
    data () {
      return {
        ...
        rules: {
          name: [
            {
              ...
              validator: (rule, value, callback) => {
                if (!value) {
                  callback(new Error('降等原因不能为空'))
                } else {
                  if (value.length > 32) {
                    callback(new Error('降等原因过长'))
                  }
          ...
          downGradeReasonTypeId: [
            { required: true, message: '降等类别不能为空', trigger: 'blur change' }
          ],
          productProcessList: [
