  ...
  <div>
    ...
    <div class="hy-admin__main-container">
      ...
      <el-table :data="tableData" border style="width: 100%" v-loading="loading" element-loading-text="拼命加载中">
        <el-table-column prop="name" label="降等原因" show-overflow-tooltip></el-table-column>
        <el-table-column prop="downGradeReasonTypeName" label="降等原因类型" show-overflow-tooltip></el-table-column>
        <el-table-column label="产品工艺" show-overflow-tooltip>
          <template slot-scope="scope">
            <el-tag
              v-for="tag in scope.row.productProcessList"
              :key="tag.id"
              class="tags">
              {{tag.name}}
            </el-tag>
        ...
        <el-table-column label="所属工种" show-overflow-tooltip>
          <template slot-scope="scope">
            <el-tag
              v-for="tag in scope.row.workTypeLsit"
              :key="tag.id"
              class="tags">
              {{tag.name}}
            </el-tag>
        ...
        <el-table-column label="所属车间" show-overflow-tooltip>
          <template slot-scope="scope">
            <el-tag
              v-for="tag in scope.row.workshopList"
              :key="tag.id"
              class="tags">
              {{tag.name}}
            </el-tag>
        ...
        <el-table-column label="产品" show-overflow-tooltip>
          <template slot-scope="scope"><el-tag v-for="(tag,index) in scope.row.productList" :key="index" class="tags">{{tag.name}}</el-tag></template>
        </el-table-column>
        <el-table-column prop="number" label="编号" show-overflow-tooltip></el-table-column>
        <el-table-column label="操作" show-overflow-tooltip>
          <template slot-scope="scope">
            <el-button @click.native.prevent="chooseFun('edit', scope)" type="text" size="small">修改</el-button>
          </template>
        </el-table-column>
      </el-table>
