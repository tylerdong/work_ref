    ...
    <el-table :data="tableData" border v-loading="loading.table" style="width: 100%">
      ...
      <el-table-column width="250" label="操作">
        <template slot-scope="scope">
          <div>
            <el-button v-if="scope.row.reason ==='退货翻包'" :disabled="scope.row.status === 'OFF' || scope.row.sapPostStatus === 'y'" @click="btnPost(scope.row)" size="small" type="primary">过账安排</el-button>
            <el-button @click="rummageClick(scope.row)" size="small" type="primary" :disabled="(scope.row.reason==='退货翻包' && scope.row.sapPostStatus === 'n') || scope.row.status === 'OFF'">翻包</el-button>
            <el-button @click="btnStatus(scope.row)" :loading="scope.row.loading" size="small" type="primary">状态切换</el-button>
          </div>
        </template>
      </el-table-column>
    </el-table>
