      ...
      <el-table :data="tableData" border style="width: 100%" v-loading="loading.table">
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            ...
            <el-button v-if="scope.row.lprio === '' && scope.row.pickup === '' && scope.row.post === ''"
                       @click="reAllot(scope.row)" type="text" :loading="loading.reAllot">重新调拨
            </el-button>

            <el-button v-if="scope.row.lprio === 'X' && scope.row.pickup === '' && scope.row.post === ''" type="text"
                       @click="asyncSAP(scope.row, 'PICKUP_FAILED')" :loading="loading.async">修改拣配
            </el-button>

            <el-button v-if="scope.row.lprio === 'X' && scope.row.pickup === '' && scope.row.post === ''"
                       @click="requisitionRepick(scope.row)" type="text" :loading="loading.pick">重新拣配
            </el-button>

            <el-button v-if="scope.row.lprio === 'X' && scope.row.pickup === 'X' && scope.row.post === ''" type="text"
                       @click="asyncSAP(scope.row, 'POST_FAILED')" :loading="loading.async">修改过账
            </el-button>

            <el-button v-if="scope.row.lprio === 'X' && scope.row.pickup === 'X' && scope.row.post === ''"
